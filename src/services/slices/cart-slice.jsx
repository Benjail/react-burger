...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    appendIngredientCart: (state, action) => { state.ingredients.push(action.payload); },
    removeCart: (state, action) => { state.ingredients = state.ingredients.filter((ingredient) => ingredient.uuid !== action.payload );},
    sortCart: (state, action) => { const prevItem = state.ingredients.find(({ uuid }) => uuid === action.payload.prevUuid );
      const newItem = state.ingredients.find(({ uuid }) => uuid === action.payload.newUuid );
